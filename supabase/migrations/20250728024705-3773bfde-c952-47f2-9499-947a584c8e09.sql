...
FOR SELECT USING (
  ...
  (get_user_role(auth.uid()) = 'admin' AND church_id = get_user_church(auth.uid()))
  OR
  -- Pastores podem ver todos da sua igreja
  (get_user_role(auth.uid()) = 'pastor' AND church_id = get_user_church(auth.uid()))
  OR
  -- Missionários podem ver seus supervisionados e supervisionados dos supervisionados
  (get_user_role(auth.uid()) = 'missionario' AND 
  ...
  (get_user_role(auth.uid()) = 'lider' AND 
...
FOR ALL USING (
  EXISTS (
    ...
    AND hg.church_id = get_user_church(auth.uid())
    AND (
      get_user_role(auth.uid()) = ANY(ARRAY['admin', 'pastor', 'missionario', 'lider'])
...
FOR ALL USING (
  ...
  AND get_user_role(auth.uid()) = ANY(ARRAY['admin', 'pastor', 'missionario', 'lider'])
);
...
FOR ALL USING (
  EXISTS (
    ...
    AND hg.church_id = get_user_church(auth.uid())
    AND (
      get_user_role(auth.uid()) = ANY(ARRAY['admin', 'pastor', 'missionario'])
      OR hg.leader_id = (SELECT id FROM profiles WHERE user_id = auth.uid())
    )
  )
);
...
FOR ALL USING (
  church_id = get_user_church(auth.uid()) 
  AND get_user_role(auth.uid()) = ANY(ARRAY['admin', 'pastor'])
);
